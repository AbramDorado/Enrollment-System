        ...
        <div class="section-top-border">
            ...
            <div class="row">
                <div class="col-lg-6 col-md-6">
                    ...
                    @guest
                        <p>If you have an account, log in with your credentials <a href="{{ route('enroll.handleLogin', $course->id) }}">here</a>.</p>
                    @endguest
                    @if(auth()->check() && auth()->user()->hasEnrolledCourse($course->id))
                        <div class="alert alert-info" role="alert">
                            You have already enrolled in this course.
                        </div>
                    @elseif(auth()->check() && auth()->user()->hasCourseInCart($course->id))
                        <div class="alert alert-info" role="alert">
                            This course is already in your cart.
                        </div>
                    @else
                        <form method="POST" action="{{ route('cart.store', $course->id) }}">
                            @csrf
                            <div class="mt-10">
                            ...
                            <div class="mt-10 pull-right">
                                <input type="submit" class="genric-btn primary" name="submit" value="Submit" @if(auth()->check() && (auth()->user()->hasEnrolledCourse($course->id) || auth()->user()->hasCourseInCart($course->id))) disabled @endif>
                            </div>
                        </form>
                    @endif
                </div>
